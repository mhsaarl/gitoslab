...
void add_commit_git() {
    // Ensure all files are added to Git staging area
    int status = system("git add .");
    if (status != 0) {
        printf("Error: Failed to add files to git staging.\n");
        return;
    }

    // Commit the changes
    status = system("git commit -m \"Updated files\"");
    if (status != 0) {
        printf("Error: Git commit failed. Ensure there are changes to commit.\n");
        return;
    }
...
// Check if there are untracked files or permission issues before git add
void check_permissions() {
    system("chmod -R 644 ."); // Ensure all files have appropriate permissions
    system("chown -R your_user:your_group ."); // Ensure proper ownership
}
...
int main() {
    ...
    // Check file permissions and ownership before proceeding with git operations
    check_permissions();
