    ...
    while(1) {
        ...
        switch(task) {
                ...
                while (1) {
                    ...
                    switch (choice) {
                        ...
                        case 2: {
                            // Write to the file
                            if (fd == -1) {
                                printf(" create or open a file first.\n");
                                break;
                            }

                            printf("Write to the file or type 'stop' to quit:\n");
                        ...
                        case 4: {
                            ...
                            mode_t new_permissions = strtol(permission, NULL, 8); // Convert string to octal mode
                        ...
                        case 5: {
                            // Exit to the menu instead of quitting
                            return 1;
                            break;
                        }
                    }
            ...
            char buffer[1024];
             ...
             printf("Parent is reading the data from the pipe. the pipe content is :\n");
             ...
             waitpid(pid2, NULL, 0); // Wait for Child 2
                  ...
                  printf("Child processes are over.\n");        
          break;
          }    

            case 4: {
                // Handle executing a command and tracking its system calls
                char strace_output[] = "strace_output.txt";
                char report_file[] = "strace_report.txt";
                pid_t pid = fork();
                if (pid == 0) {
                    // Child process runs strace on the program (e.g., ls)
                    execlp("strace", "strace", "-o", strace_output, "ls", NULL);
                    perror("exec failed");
                    exit(EXIT_FAILURE);
                }
                wait(NULL); // Wait for strace to complete
                analyze_strace_output(strace_output, report_file);
                break;
            }
