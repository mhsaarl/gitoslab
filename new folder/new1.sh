...
send_nonintrusive_signals() {
    ...
    echo -e "\nSending non-intrusive signals to process with PID: $pid"
      kill -SIGLOST $pid
    echo "Sent SIGLOST to process $pid"
      kill -SIGCONT $pid
    echo "Sent SIGCONT to process $pid"
      kill -SIGILL $pid
    echo "Sent SIGILL to process $pid"
      kill -SIGPOLL $pid
    echo "Sent SIGPOLL to process $pid"
        kill -SIGIO $pid
    echo "Sent SIGIO to process $pid"       
}
